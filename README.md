...
- 도메인 클래스를 `domain` 패키지로 이동
    - `Information`, `Member`, `Review` 클래스를 `com.eomcs.lms.domain` 패키지로 옮긴다.
    - 다른 패키지에서 변수에 접근할 수 있도록 접근 제어를 `public` 으로 변경
- 핸들러 클래스에 import 문 추가
    - `InformationHandler`, `MemberHandler`, `ReviewHandler` 클래스를 변경한다.
...
- 핸들러 클래스를 `handler` 패키지로 이동
    - `InformationHandler`, `MemberHandler`, `ReviewHandler` 클래스를 `com.eomcs.lms.handler` 패키지로 옮긴다.
    - 다른 패키지에서 변수에 접근할 수 있도록 접근 제어를 `public` 으로 변경
- App.java 변경
